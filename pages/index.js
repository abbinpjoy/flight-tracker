...
function PriceGrid({ origin, destination, baseDate, retDate, cabin, passengers }) {
  ...
  // Run async tasks with bounded concurrency; resolves like Promise.allSettled
  async function pooledFetch(taskFns, limit = 3) {
  ...
  async function fetchGrid() {
    ...
    try {
      if (!isRoundTrip) {
        ...
        // skipDuffel/skipVI: a 7-cell grid would otherwise fire 7 Duffel
        // searches + 7×(up to 10) VI hub searches — instant rate-limit bans.
        // Pool of 3 keeps SerpAPI happy and the function under timeout.
...
function RouteTracker({ route, onUpdate, alerts, alertEmail, addLog, firedAlertsRef }) {
  ...
  // and the log was stuck on "Tick #1". The interval now calls through a ref
  // that always points at the latest doFetch.
  ...
  const doFetch = useCallback(async () => {
    setLoading(true)
    tickRef.current += 1
    addLog('info', `[${origin}→${destination}] Tick #${tickRef.current} searching…`)
    try {
      const now              = Date.now()
    ...
    setLoading(false)
  }, [origin, destination, depDate, retDate, cabin, passengers, minLayover, alerts, alertEmail, id, addLog])

  // Keep the ref pointing at the freshest closure on every render
  doFetchRef.current = doFetch

  const startTracking = useCallback(async () => {
    setIsTracking(true); setTickCount(0); tickRef.current = 0
    ...
    timerRef.current = setInterval(() => doFetchRef.current?.(), refreshSecs*1000)
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ display:'flex', gap:6, alignItems:'center', marginTop:4, flexWrap:'wrap' }}>
                          ...
                          {f.isVirtualInterline
                            ...
                            : f.stops==null
                              ? <Badge color="grey" small>stops n/a</Badge>
                              : f.stops===0
                                ? <Badge color="green" small>Direct</Badge>
                                : <Badge color="amber" small>{f.stops} stop{f.stops>1?'s':''}{f.via?` via ${f.via}`:''}</Badge>
                          }
                          {f.source==='estimate' && <Badge color="red" small>⚠ ESTIMATE — not a live fare</Badge>}
...
export default function FlightTracker() {
  ...
  useEffect(() => {
    ...
    // Restore saved routes — previously routes were lost on every page refresh
    const savedRoutes = ls.get('ft_routes', null)
    if (Array.isArray(savedRoutes) && savedRoutes.length) {
      const valid = savedRoutes.filter(r => r && r.id && r.origin && r.destination && r.depDate)
      if (valid.length) { setRoutes(valid); setActiveRoute(valid[0].id) }
    }
  ...
  useEffect(() => { ls.set('ft_routes', routes) }, [routes])
