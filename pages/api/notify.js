...
export default async function handler(req, res) {
  ...
  // ── Anti-abuse: this endpoint is publicly reachable on Vercel. Without a
  // recipient allowlist, anyone can use it to send spam from your Resend
  // account/domain. Set ALERT_ALLOWED_EMAILS (comma-separated) in Vercel.
  const allowed = (process.env.ALERT_ALLOWED_EMAILS || '')
    .split(',').map(e => e.trim().toLowerCase()).filter(Boolean)
  if (allowed.length && !allowed.includes(String(to).toLowerCase())) {
    return res.status(403).json({ sent: false, reason: 'Recipient not in ALERT_ALLOWED_EMAILS' })
  }
