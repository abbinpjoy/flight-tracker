...
export async function searchVirtualInterline({
  ...
  const BATCH_SIZE  = returnDate ? 1 : 2  // RT = 4 calls per hub, keep ≤4 concurrent
  ...
  for (let i = 0; i < hubs.length; i += BATCH_SIZE) {
    ...
    for (const hub of batch) {
      ...
      if (returnDate) {
        batchSearches.push(
          client.searchOffers({
            origin: destination, destination: hub,
            departureDate: returnDate, adults: pax,
            cabinClass: cabin, currency, minLayoverMins: 0,
          }).then(r => ({ hub, leg: 'retOut', offers: (r||[]).filter(o=>o.price>0) }))
            .catch(e => { console.warn(`[VI] ${destination}→${hub} (ret): ${e.message}`); return { hub, leg:'retOut', offers:[] } })
        )
        batchSearches.push(
          client.searchOffers({
            ...
            departureDate: returnDate, adults: pax,
            cabinClass: cabin, currency, minLayoverMins: 0,
          }).then(r => ({ hub, leg: 'retIn', offers: (r||[]).filter(o=>o.price>0) }))
            .catch(e => { console.warn(`[VI] ${hub}→${origin} (ret): ${e.message}`); return { hub, leg:'retIn', offers:[] } })
        )
      }
    }
    ...
    for (const r of batchResults) {
      ...
      if (!hubMap[hub]) hubMap[hub] = { out:[], in:[], retOut:[], retIn:[] }
