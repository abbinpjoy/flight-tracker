...
export async function searchTravelpayouts({
  ...
  try {
    ...
    for (const [, ticket] of Object.entries(destData)) {
      ...
      const priceCAD = convert(ticket.price, 'USD', 'CAD', fxRates)
      ...
      flights.push({
        ...
        duration:     '—',
        durationMins: 0,
        stops:        null, // UNKNOWN — /v1 cache doesn't expose stop count; was 0 (false 'Direct' claims)
        via:          null,
        segments:     [],
        minLayoverMins: null,
        maxLayoverMins: null,
    ...
    if (v2res.ok) {
      ...
      if (v2json.success && Array.isArray(v2json.data)) {
        for (const item of v2json.data) {
          ...
          const priceCAD = convert(item.value, 'USD', 'CAD', fxRates)
