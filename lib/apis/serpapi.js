...
export async function searchGoogleFlights({
  ...
  try {
    ...
    // SerpAPI returns HTTP 200 with an in-body error for bad keys, exhausted
    // quota ("You have run out of searches"), or invalid params. Previously
    // this fell through to "0 results" and looked like no flights existed.
    if (data.error) throw new Error(`SerpAPI: ${data.error}`)

    // Google Flights URL that loads the same search results
    // This is provided by SerpAPI in search_metadata.google_flights_url
    ...
    for (const offer of allOffers) {
      try {
        ...
        // ── FIX: 0-price bug ────────────────────────────────────────────
        // SerpAPI sometimes returns price=0 when:
        //   - offer.price is missing (use extracted_price as fallback)
        //   - flight is "Price unavailable" on Google Flights
        // Skip flights with invalid/zero prices entirely
        const rawPrice = offer.price || offer.extracted_price || 0
        if (!rawPrice || rawPrice <= 0) {
          console.log(`[SerpAPI] Skipped 0-price offer (${first.airline || 'unknown'})`)
          continue
        }

        // Currency conversion — only if SerpAPI explicitly echoes a DIFFERENT
        // currency than requested. Previously the fallback default was 'USD',
        // which double-converted already-CAD prices (×1.37) whenever the echo
        // field was missing. Default to the REQUESTED currency instead.
        const responseCurrency = data.search_parameters?.currency || currency
        let price = rawPrice
        if (responseCurrency !== currency) {
          price = convert(rawPrice, responseCurrency, currency, fxRates)
        }
        ...
        flights.push({
          ...
          // Store the exact Google Flights URL that loads this search
          // /api/book will use this to redirect users to the actual search page
          bookUrl:       googleFlightsUrl,
          googleFlightsUrl,
    ...
    // Re-throw so the orchestrator reports status:'error' with the message —
    // a silent null rendered as 'skipped' and hid quota/key problems from the UI
    throw err
  }
}
...
function extractCode(logoUrl) {
  ...
  const m = logoUrl.match(/\/([A-Z0-9]{2})\.png/)
  return m ? m[1] : ''
}
