...
export async function searchApifyGoogleFlights({
  ...
  const actor = (process.env.APIFY_GFLIGHTS_ACTOR || 'johnvc~google-flights-data-scraper-flight-and-price-search')
    .replace('/', '~') // accept either separator

  // Input per the actor's documented schema (johnvc). Round trip is
  // inferred from the presence of return_date. No cabin field exists —
  // this actor searches economy fares.
  const input = {
    departure_id:  origin,
  ...
  let items
  const t0 = Date.now()
  console.log(`[Apify] Running actor ${actor} for ${origin}→${destination} ${date}${returnDate ? '⇄'+returnDate : ''}`)
  try {
    ...
    console.log(`[Apify] Actor finished in ${((Date.now()-t0)/1000).toFixed(1)}s, ${Array.isArray(items) ? items.length : 0} dataset item(s)`)
  } catch (err) {
...
function normalizeOffer(offer, { currency, fxRates, minLayoverMins }) {
  ...
  // Parse "5h 30m" / "16h 5m" style duration strings → minutes
  const parseDurStr = v => {
  ...
  // Airports may be a string code ("LAX") or an object ({ id, time })
  const airportCode = v => typeof v === 'string' ? v : (v?.id || v?.iata || v?.code || '')

  const first = legs[0] || {}
  const last  = legs[legs.length - 1] || first
  // Top-level stops (johnvc) beats leg-count inference (mirror)
  const stops = get(offer, 'stops', 'number_of_stops') ?? (legs.length ? legs.length - 1 : null)
  ...
  // Prefer TOP-LEVEL fields (johnvc shape), fall back to leg objects (mirror shape)
  const depAirport = first.departure_airport || first.departureAirport || {}
  const arrAirport = last.arrival_airport   || last.arrivalAirport    || {}
  const departure  = timeOf(get(offer, 'departure_time') || depAirport.time || get(first, 'departure_time', 'departureTime', 'departing_at')) || '—'
  const arrival    = timeOf(get(offer, 'arrival_time')   || arrAirport.time || get(last,  'arrival_time',  'arrivalTime',  'arriving_at'))   || '—'

  const durMins = parseDurStr(get(offer, 'duration', 'total_duration', 'totalDuration', 'duration_minutes', 'durationMins')) ||
                  legs.reduce((s, l) => s + parseDurStr(l.duration), 0) + layoverDurations.reduce((s, m) => s + m, 0) || 0

  const segments = legs.map((leg, i) => ({
    from:         airportCode(leg.departure_airport || leg.departureAirport) || get(leg, 'from', 'origin') || '',
    to:           airportCode(leg.arrival_airport   || leg.arrivalAirport)   || get(leg, 'to', 'destination') || '',
    ...
    durationMins: parseDurStr(leg.duration),
