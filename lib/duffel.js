...
import { getRatesToCAD, convert, STATIC_RATES_TO_CAD } from './fx.js'
...
export class DuffelClient {
  ...
  async request(method, path, body = null, retries = 2) {
    ...
    if (!res.ok) {
      ...
      // Duffel sends this as an HTTP DATE string (e.g. "Wed, 11 Jun 2026 15:02:11 GMT"),
      // not seconds — parseInt(date) is NaN, which made waitMs NaN and caused
      // instant retries that burned all attempts in milliseconds.
      if (res.status === 429 && retries > 0) {
        const resetHdr = res.headers.get('ratelimit-reset') || ''
        let waitMs
        const asNum = parseFloat(resetHdr)
        if (Number.isFinite(asNum) && asNum < 3600) {
          waitMs = asNum * 1000                      // header was seconds
        } else {
          const asDate = Date.parse(resetHdr)
          waitMs = Number.isFinite(asDate) ? asDate - Date.now() : NaN  // header was a date
        }
        if (!Number.isFinite(waitMs) || waitMs <= 0) waitMs = 0
        waitMs = Math.min(Math.max(waitMs, 2000), 30000) // clamp 2s–30s
  ...
  async searchOffers({
    ...
    // Live FX rates (12h-cached in fx.js, static fallback offline)
    const rates = await getRatesToCAD()

    return this.normalizeOffers(raw, minLayoverMins, maxLayoverMins, currency, rates)
  }

  normalizeOffers(offers, minLayoverMins, maxLayoverMins, outputCurrency, fxRates = null) {
    const results = []
    ...
    for (const offer of offers) {
      try {
        ...
        // Centralized FX (lib/fx.js): live rates when available, static fallback otherwise
        const price    = rawCurr === outputCurrency
          ? Math.round(rawPrice)
          : convert(rawPrice, rawCurr, outputCurrency, fxRates || STATIC_RATES_TO_CAD)
        ...
        results.push({
          ...
          // Raw local timestamps — used by Virtual Interline to verify the
          // inbound leg actually departs AFTER the outbound leg arrives at the hub
          departingAtISO: first.departing_at || null,
          arrivingAtISO:  last.arriving_at   || null,
          source:        'duffel',
        })
      } catch (e) {
