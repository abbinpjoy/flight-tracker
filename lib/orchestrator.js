...
import { searchApifyGoogleFlights } from './apis/apify-gflights.js'
...
const T = {
  apify:     50000, // actor cold-start can take 10–30s; runs inside maxDuration 60
  kiwi:      12000,
  duffel:    15000,
  travelpayouts: 10000,
  virtualinterline: 50000, // two paced stages × up to 2 batches × Duffel latency
  agent:     22000,
}
...
export async function orchestrateSearch({
  ...
  const hasApify        = !!(process.env.APIFY_TOKEN?.length > 10) && !skipApify
  ...
  const hasAgent        = !!apiKey && !hasApify && !hasKiwi && !hasDuffel && !hasTravelpayouts

  log(`APIs — GoogleFlights(Apify):${hasApify}${skipApify?'(skip)':''} TP:${hasTravelpayouts} Duffel:${hasDuffel}${skipDuffel?'(skip)':''} VI:${hasVI}${skipVI?'(skip)':''} Kiwi:${hasKiwi} Agent:${hasAgent}`)

  const params     = { origin, destination, date, returnDate, cabin, passengers, currency, minLayoverMins, maxLayoverMins }
  const viCacheKey = `${origin}-${destination}-${date}-${returnDate||'ow'}-${cabin}-${passengers}`
  ...
  const [rSerp, rKiwi, rDuffel, rTP, rVI, rAgent] = await Promise.allSettled([
    hasApify
      ? raceTimeout(searchApifyGoogleFlights(params), T.apify, 'GoogleFlights')
      ...
      ? cachedVI
        ...
        : raceTimeout(
            new Promise(r => setTimeout(r, 2000))
              .then(() => searchVirtualInterline({ origin, destination, date, returnDate, cabin, passengers, minLayoverMins, currency }))
  ...
  // 'Direct' must come from a source that returns real itineraries with segments.
  // Travelpayouts v1 cache and estimates can't prove a flight is nonstop.
  const direct   = scored.find(f =>
    f.stops === 0 && !f.isVirtualInterline &&
    f.source !== 'travelpayouts' && f.source !== 'estimate' &&
