...
import { getRatesToCAD, STATIC_RATES_TO_CAD } from './fx.js'
...
export async function agentSearch({
  ...
  // Live FX for the prompt — stale hardcoded rates silently mis-rank prices
  let fx = STATIC_RATES_TO_CAD
  try { fx = await getRatesToCAD() } catch {}
  const fxLine = ['USD','GBP','EUR','AED','INR','SGD','AUD','QAR']
    .map(c => `1 ${c}=${(fx[c] ?? STATIC_RATES_TO_CAD[c]).toFixed(c==='INR'?4:2)}`)
    .join(', ')
...
Convert all prices to CAD using today's rates: ${fxLine}.
